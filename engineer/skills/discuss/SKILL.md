...
4. **Surface the outcome** — at a natural inflection, recommend an outcome (build / park / drop); the user confirms. Never auto-execute. If the user says "drop" but there's a concrete reason it's worth parking, ask once. **Present it as a choice point** (`${CLAUDE_PLUGIN_ROOT}/references/choice-points.md`): recommend one option marked "(Recommended)" with a one-line rationale, alternatives listed, `autonomy_level`-dialed default — not an open question. When the outcome is **build**, the recommendation carries the two funnel decisions, both projected from the conversation (size + clarity, per `choice-points.md`):
   - **A — front half:** shape known → **spec-first**; fuzzy / UX-heavy / "I'll know it when I see it" → **prototype-first** (`${CLAUDE_PLUGIN_ROOT}/references/two-paths.md`) — iterate a rough artifact, convert to spec later.
   - **B — weight:** projected ~1-PR / low-risk → **express (XS)** (`${CLAUDE_PLUGIN_ROOT}/references/express-lane.md`, one pass); non-trivial → **full DAE**. Charter-flagged path is never recommended express (safety wins). Prototype-first defers B to convergence (its `prototype_disposition`).
   Carry the projected `size` into the promote/prototype handoff so `feature-init` need not re-ask.
